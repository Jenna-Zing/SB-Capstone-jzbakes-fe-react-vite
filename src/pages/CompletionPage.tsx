...
const STATUS_CONTENT_MAP: Record<string, { text: string; iconColor: string; icon: React.ReactNode }> = {
  succeeded: {
...
export default function CompletionPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
      ...
      >
        <div className="scale-150">{statusContent.icon}</div>
      </div>
      
      <h2 className="text-3xl font-bold mb-6 text-gray-800">{statusContent.text}</h2>
      
      {intentId && (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 max-w-md w-full mb-8">
          <table className="w-full">
            <tbody className="divide-y divide-gray-100">
              <tr>
                <td className="py-3 text-left font-medium text-gray-500">Transaction ID</td>
                <td className="py-3 text-right font-mono text-sm text-gray-700 break-all">{intentId}</td>
              </tr>
              <tr>
                <td className="py-3 text-left font-medium text-gray-500">Status</td>
                <td className="py-3 text-right font-medium text-gray-900 capitalize">{status}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="flex flex-col gap-4 w-full max-w-xs">
        ...
        <Link 
          to="/" 
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded transition-colors text-center"
        >
          Return to Shop
        </Link>
      </div>
